...
class LspClientK(process: Process)(using ExecutionContext):
  ...
  private val shutdownRequested = AtomicBoolean.init(false)
  ...
  private def readMessageBody(
    ...
    def readLoop(currentBuffer: String): (updatedBuffer: String, success: Boolean, shouldContinue: Boolean) < (IO & Async) =
      if currentBuffer.length >= headerEnd + 4 + contentLength then
        ...
        for
          ...
          result <- moreBytesRead match
            ...
            case 0 =>
              Async.delay(10.millis)(readLoop(currentBuffer))
            case n =>
  ...
  def readMessages(): Unit < (Async & Log & Abort[Exception]) =

    def mainLoop(buffer: String): Unit < (Async & Log & Abort[Exception]) =
      shutdownRequested.map(_.get).map: requested =>
        if requested || !process.isAlive then 
          Kyo.unit
        else
          for
          ...
          parseContentLength(header) match
            case Some(contentLength) if contentLength > 0 =>
              for
                ...
                _ <-
                  if !shouldContinueAfterRead then 
                    Kyo.unit
  ...
  private def parseAndHandleMessageKyo(messageJson: String) =
    for
