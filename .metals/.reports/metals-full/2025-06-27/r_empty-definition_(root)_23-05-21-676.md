...
non-local guesses:
	 ...
	 -io/circe/requested#
	 -io/circe/requested().
	 -io/circe/parser/requested.
	 -io/circe/parser/requested#
	 -io/circe/parser/requested().
	 -io/circe/syntax/requested.
	 -io/circe/syntax/requested#
	 -io/circe/syntax/requested().
	 -kyo/requested.
	 -kyo/requested#
...
class LspClientK(process: Process)(using ExecutionContext):
  ...
  def readMessages(): Unit < (IO & Log & Async & Abort[Exception]) =
    ...
    def mainLoop(buffer: String): Unit < (IO & Log & Async & Abort[Exception]) =
      for 
        reques@@ted <- shutdownRequested.get
      yield ()
      shutdownRequested.map: requested =>
        if requested.get || !process.isAlive then ()
        else
          for
